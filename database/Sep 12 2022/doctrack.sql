...
(1,'Proposed Ordenances',1,1),
(2,'Proposed Resolutions',1,1),
(4,'Petitions and Request',1,1),
(5,'Other Communications Subject for Legislation',1,1);
...
CREATE TABLE `tmp_order_of_business` (
  ...
  `ordinance_code` varchar(50) NOT NULL,
...
/*Table structure for table `vw_gen_order_business` */

DROP TABLE IF EXISTS `vw_gen_order_business`;

/*!50001 DROP VIEW IF EXISTS `vw_gen_order_business` */;
/*!50001 DROP TABLE IF EXISTS `vw_gen_order_business` */;

/*!50001 CREATE TABLE  `vw_gen_order_business`(
 `row1` int(11) ,
 `row2` varchar(100) ,
 `row3` varchar(100) ,
 `row4` varchar(50) ,
...
/*View structure for view vw_gen_order_business */

/*!50001 DROP TABLE IF EXISTS `vw_gen_order_business` */;
/*!50001 DROP VIEW IF EXISTS `vw_gen_order_business` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_gen_order_business` AS select `a`.`id` AS `row1`,`a`.`barcode` AS `row2`,`b`.`order_of_business` AS `row3`,`a`.`title` AS `row4`,`a`.`ordinance_code` AS `row5`,`a`.`description` AS `row6`,`a`.`added_date` AS `row7`,`func_fullname`(`a`.`added_by`) AS `row8`,`a`.`added_by` AS `row9` from (`tmp_order_of_business` `a` left join `tbl_setup_order_of_business` `b` on(`b`.`id` = `a`.`order_of_business_id`)) */;
